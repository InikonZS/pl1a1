import { AmbientLight, AnimationMixer, DirectionalLight, LoopOnce, LoopRepeat, Mesh, PointLight, Vector3, type Group, type Scene } from "three";
...
export class Cat {
    ...
    constructor(scene: Scene, world: World) {
        ...
        loader.load(
            './cat.glb', // Путь к файлу из папки public
            (gltf) => {
                this.gltf = gltf;
                ...
                console.log(gltf)
                const action = this.animationMixer.clipAction(gltf.animations.find(it=>it.name == 'Action_walk2'));
                action.setLoop(LoopRepeat, Infinity);
                //action.play();
                 this.actionWalk= action;

                 const actionIdle = this.animationMixer.clipAction(this.gltf.animations.find(it=>it.name == 'Action_sit'));
            this.animationMixer.stopAllAction();
                actionIdle.setLoop(LoopOnce, 0);
                actionIdle.clampWhenFinished = true;
                actionIdle.play();
                ...
                dirLight.position.set(1.2, +3.2, 0);
        ...
        this.boxBody = new Body({
            ...
            position: new Vec3(6, 3, -1), // Поднимаем на высоту 5 метров
    ...
    animate(){
        if (this.loadedModel) {
            this.loadedModel.position.set(this.boxBody.position.x, this.boxBody.position.y - 0.95, this.boxBody.position.z);
    ...
    input(data: IPointer){
        ...
        const forward = new Vector3(data.result.x / 20, 0, data.result.y / 20);
